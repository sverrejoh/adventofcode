...
const part_one = (input: string) => {
  ...
  for (let y = 0; y < lines[0].length; y++) {
    let word: { content: string; isAdjacent: boolean } = {
      content: [],
      isAdjacent: false,
    };
    for (let x = 0; x <= lines.length; x++) {
      ...
      if (isNaN(parseInt(char, 10))) {
        if (word.content.length && word.isAdjacent) {
          sum += parseInt(word.content.join(""), 10);
        }
        word = { content: [], isAdjacent: false };
      } else {
        if (isSymbol(lines?.[y]?.[x - 1], false)) {
          word.isAdjacent = true;
        } else if (isSymbol(lines?.[y + 1]?.[x - 1])) {
          word.isAdjacent = true;
        } else if (isSymbol(lines?.[y + 1]?.[x])) {
          word.isAdjacent = true;
        } else if (isSymbol(lines?.[y + 1]?.[x + 1])) {
          word.isAdjacent = true;
        } else if (isSymbol(lines?.[y]?.[x + 1], false)) {
          word.isAdjacent = true;
        } else if (isSymbol(lines?.[y - 1]?.[x + 1])) {
          word.isAdjacent = true;
        } else if (isSymbol(lines?.[y - 1]?.[x])) {
...
const part_two = (input: string) => {
  const lines = input.split("\n").map((line) => line.split(""));
  const gears: { [key: string]: { content: string[] }[] } = {};
  for (let y = 0; y < lines[0].length; y++) {
    let word: {
      content: string[];
      gear?: { x: number; y: number } | undefined;
    } = {
      content: [],
    };
    for (let x = 0; x <= lines.length; x++) {
      ...
      if (isNaN(parseInt(char, 10))) {
        ...
        if (isGear(lines?.[y]?.[x - 1])) {
          word.gear = { x: x - 1, y };
        } else if (isGear(lines?.[y + 1]?.[x - 1])) {
          word.gear = { x: x - 1, y: y + 1 };
        } else if (isGear(lines?.[y + 1]?.[x])) {
          word.gear = { x: x, y: y + 1 };
        } else if (isGear(lines?.[y + 1]?.[x + 1])) {
          word.gear = { x: x + 1, y: y + 1 };
        } else if (isGear(lines?.[y]?.[x + 1])) {
          word.gear = { x: x + 1, y: y };
        } else if (isGear(lines?.[y - 1]?.[x + 1])) {
          word.gear = { x: x + 1, y: y - 1 };
        } else if (isGear(lines?.[y - 1]?.[x])) {
  ...
  return Object.entries(gears).reduce((sum, [key, gears]) => {
    if (gears.length > 1) {
      return (
        sum + gears.reduce((k, v) => k * parseInt(v.content.join(""), 10), 1)
      );
    } else {
      return sum;
    }
...
.664.598..`;
  ...
  it(`solves ${name(__filename)}, part one with example data`, () => {
    expect(part_one(EXAMPLE_1)).toBe(4361);
  });

  const EXAMPLE_2 = `467..114..
...*......
..35..633.
......#...
617*......
...
.664.598..`;
  ...
  it(`solves ${name(__filename)}, part two with example data`, () => {
    expect(part_two(EXAMPLE_2)).toBe(467835);
  });
}

const INPUT = `...766.......821.547.....577......................................387.....................56..........446.793..........292..................
